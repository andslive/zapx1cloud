...
export default function Login() {
  ...
  useEffect(() => {
    ...
    if (sessionStorage.getItem('superadmin_bootstrap_attempted') === '1') return;
    sessionStorage.setItem('superadmin_bootstrap_attempted', '1');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isLogin) {
        console.log('[LOGIN_UI_SUBMIT]', { email });
        const { error } = await signIn(email, password);
        if (error) {
          const errorMessage = translateAuthError(error);
          console.error('[LOGIN_UI_ERROR]', { error, translated: errorMessage });
