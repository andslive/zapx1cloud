-- Corrige exclusão de lead falhando (FK violation) quando a conversa do lead
-- tem linhas em conversation_processing_leases ou receipt_recovery_requests.
-- Ambas referenciam webchat_conversations.id com ON DELETE NO ACTION e não
-- eram limpas por delete_lead_cascade antes do DELETE FROM webchat_conversations,
-- derrubando a exclusão (e, em seleção múltipla, o lote inteiro, já que o loop
...
BEGIN
  ...
  LOOP
    ...
    IF _conv_ids IS NOT NULL THEN
      ...
      DELETE FROM public.conversation_processing_leases WHERE conversation_id = ANY(_conv_ids);
      DELETE FROM public.receipt_recovery_requests WHERE conversation_id = ANY(_conv_ids);
