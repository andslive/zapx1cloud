// deno test --allow-import conversation-isolation-flags.test.ts
//
// Mesma regra e mesmo mock de meta-cloud-flags.test.ts (padrão já em
// produção): desligada por padrão, globalmente E por organização. Linha de
// organização tem prioridade sobre a global quando presente.

import { assertEquals } from "https://deno.land/std@0.208.0/assert/mod.ts";
import { isConversationIsolationEnabled } from "./conversation-isolation-flags.ts";
...
function fakeSupabase(rows: FlagRow[]): SupabaseLike {
  return {
    from(table: string) {
      if (table !== "conversation_isolation_feature_flags") throw new Error(`tabela inesperada: ${table}`);
      return {
...
Deno.test("sem nenhuma linha (global ou org) => desligada (comportamento legado por padrão)", async () => {
  const supabase = fakeSupabase([]);
  assertEquals(await isConversationIsolationEnabled(supabase, "org-a"), false);
});
...
Deno.test("apenas global=true, sem linha de organização => usa global", async () => {
  ...
  assertEquals(await isConversationIsolationEnabled(supabase, "org-a"), true);
});
...
Deno.test("global=false mas organização piloto tem override true => ligada só para essa organização (rollout gradual)", async () => {
  const supabase = fakeSupabase([
    { scope: "global", organization_id: null, enabled: false },
    { scope: "organization", organization_id: "org-piloto", enabled: true },
  ]);
  assertEquals(await isConversationIsolationEnabled(supabase, "org-piloto"), true);
  assertEquals(await isConversationIsolationEnabled(supabase, "org-outra"), false);
});

Deno.test("organizationId nulo consulta só a flag global", async () => {
  const supabase = fakeSupabase([{ scope: "global", organization_id: null, enabled: true }]);
  assertEquals(await isConversationIsolationEnabled(supabase, null), true);
});
