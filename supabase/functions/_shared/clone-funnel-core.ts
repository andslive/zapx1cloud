...
// Fase 4B — extração pura do filtro/classificação de mensagens já usado em
// clone-funnel/index.ts (seleção das mensagens do lead que entram no funil
// clonado). Nenhuma mudança de comportamento: mesma lógica exata, só movida
// para fora do handler HTTP para ser testável sem client Supabase/rede.
// `Array.prototype.filter` preserva a ordem relativa dos elementos (garantia
// da linguagem) — por isso a ordem de entrada (já vinda ordenada por
// created_at/id da query) é preservada na saída sem nenhum passo de
...
export interface SelectEligibleLeadMessagesResult<T extends RawMessageLike> {
  eligible: T[];
  ambiguous: T[];
}

/**
 * Seleciona, na mesma ordem de entrada, apenas as mensagens inbound do
 * visitante (lead) — nunca mensagens enviadas pela instância/atendente/bot.
 * Registros com sinais de direção/autoria contraditórios (ambíguos) são
 * excluídos de `eligible` e reportados separadamente, nunca incluídos
 * silenciosamente.
 */
export function selectEligibleLeadMessages<T extends RawMessageLike>(
  messages: readonly T[],
): SelectEligibleLeadMessagesResult<T> {
  ...
  const ambiguous = messages.filter((m) => {
